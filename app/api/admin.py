...
def assessments(session: Session = Depends(get_session)) -> list[dict]:
    return [
        {"id": a.id, "key": a.key, "name": a.name, "stage": a.stage}
        for a in session.execute(select(Assessment)).scalars()
    ]
...
def submissions(session: Session = Depends(get_session)) -> list[dict]:
    return [
        {"id": s.id, "created_at": s.created_at.isoformat(), "outcome": s.result.get("outcome")}
        for s in session.execute(select(Submission)).scalars()
    ]
...
def replay(delivery_id: str, session: Session = Depends(get_session)) -> dict:
    ...
    if not delivery:
        return {"queued": False, "status": "missing"}

    delivery.status = "pending"
    session.commit()
    status = process_delivery(session, delivery_id)
